...
namespace BitMEX.Net.Clients.ExchangeApi
{
    ...
    internal partial class BitMEXSocketClientExchangeApi : SocketApiClient, IBitMEXSocketClientExchangeApi
    {
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXTradeUpdate>>(_logger, symbols.Select(x => "trade:" + x).ToArray(), x => onMessage(x.WithSymbol(x.Data.First().Symbol)), false);
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXAggTrade>>(_logger, symbols.Select(x => "tradeBin" + EnumConverter.GetString(period) + ":" + x).ToArray(), x => onMessage(x.WithSymbol(x.Data.First().Symbol)), false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXBookTicker>>(_logger, symbols.Select(x => "quote:" + x).ToArray(), x => onMessage(x.As(x.Data.First()).WithSymbol(x.Data.First().Symbol)), false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXBookTicker>>(_logger, symbols.Select(x => "quoteBin" + EnumConverter.GetString(period) + ":" + x).ToArray(), x => onMessage(x.As(x.Data.First()).WithSymbol(x.Data.First().Symbol)), false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXSettlementHistory>>(_logger, ["settlement"], onMessage, false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXOrderBookUpdate>>(_logger, symbols.Select(x => "orderBook10:" + x).ToArray(), x => onMessage(x.As(x.Data.First()).WithSymbol(x.Data.First().Symbol)), false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToIncrementalOrderBookUpdatesAsync(IEnumerable<string> symbols, IncrementalBookLimit limit, Action<DataEvent<BitMEXOrderBookIncrementalUpdate>> onMessage, CancellationToken ct = default)
        {
            var subscription = new BitMEXRawSubscription<SocketUpdate<IEnumerable<BitMEXOrderBookEntry>>>(_logger, symbols.Select(x => "orderBookL2" + (limit == IncrementalBookLimit.Top25 ? "_25": "") + ":" + x).ToArray(), x => onMessage(x.As(new BitMEXOrderBookIncrementalUpdate
            {
                ...
                Entries = x.Data.Data
            }).WithSymbol(x.Data.Data.First().Symbol).WithUpdateType(x.Data.Action == "partial" ? SocketUpdateType.Snapshot : SocketUpdateType.Update)), false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXLiquidation>>(_logger, ["liquidation"], x => onMessage(x.WithSymbol(x.Data.First().Symbol)), false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXInsurance>>(_logger, ["insurance"], onMessage, false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXOptionalSymbolSubscription<BitMEXSymbolUpdate>(_logger, "instrument", category == null ? null :[EnumConverter.GetString(category)], null, onMessage, false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXOptionalSymbolSubscription<BitMEXSymbolUpdate>(_logger, "instrument", null, [symbol], x => onMessage(x.As(x.Data.Single())), false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXOptionalSymbolSubscription<BitMEXSymbolUpdate>(_logger, "instrument", null, symbols.ToArray(), onMessage, false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXFundingRate>>(_logger, symbols.Select(x => "funding:" + x).ToArray(), x => onMessage(x.As(x.Data.First()).WithSymbol(x.Data.First().Symbol)), false);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXAnnouncement>>(_logger, ["announcement"], onMessage, false);
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXBalance>>(_logger, ["wallet"], onMessage, true);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXTransaction>>(_logger, ["transact"], onMessage, true);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXPosition>>(_logger, ["position"], onMessage, true);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXMarginStatus>>(_logger, ["margin"], x => onMessage(x.As(x.Data.First())), true);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXOrder>>(_logger, ["order"], onMessage, true);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
        ...
        {
            var subscription = new BitMEXSubscription<IEnumerable<BitMEXExecution>>(_logger, ["execution"], onMessage, true);
            return await SubscribeAsync(BaseAddress.AppendPath("realtime"), subscription, ct).ConfigureAwait(false);
        }
